  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setModal({ capacidad: 4 })} className="flex items-center gap-2 bg-primary text-darkBg font-bold px-4 py-2 rounded-lg text-sm">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {mesas.map(m => (
          <div key={m.id} className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col items-center">
            ...
            <span className={`text-xs px-2 py-1 rounded-full font-medium mb-4 ${estadoColor[m.estado]}`}>{m.estado}</span>
            <div className="flex gap-2">
              <button onClick={() => setModal(m)} className="p-1.5 bg-gray-100 rounded-lg hover:bg-gray-200 transition text-gray-600"><Pencil size={14} /></button>
              <button onClick={() => eliminar(m.id)} className="p-1.5 bg-red-500/20 rounded-lg hover:bg-red-500/40 transition text-red-400"><Trash2 size={14} /></button>
            </div>
          </div>
        ))}
      </div>
