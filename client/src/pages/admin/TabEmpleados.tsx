...
export const TabEmpleados = () => {
  ...
  const [viewMode, setViewMode] = useState<"grid" | "list">(
    (sessionStorage.getItem("empleados_view") as any) || "grid",
  );

  const [page, setPage] = useState(
    Number(sessionStorage.getItem("empleados_page")) || 1,
  );
  ...
  const cargar = useCallback(async () => {
    try {
      const { data: res } = await api.get(
        `/administrativo/empleados?page=${page}&limit=${limit}`,
      );
  ...
  const guardar = async () => {
    ...
    try {
      if (modal.id)
        await api.put(`/administrativo/empleados/${modal.id}`, modal);
        ...
        <div className="flex gap-2">
          <div className="flex bg-gray-100 p-1 rounded-lg">
            <button
              ...
              className={`p-1.5 rounded ${viewMode === "list" ? "bg-white shadow-sm text-secondary" : "text-gray-400"}`}
            >
              <List size={18} />
            </button>
            <button
              onClick={() => updateView("grid")}
              className={`p-1.5 rounded ${viewMode === "grid" ? "bg-white shadow-sm text-secondary" : "text-gray-400"}`}
            >
              <LayoutGrid size={18} />
            </button>
          </div>
          <button
            onClick={() => setModal({ nombre: "", salario_base: 0, activo: 1 })}
      ...
      {viewMode === "list" ? (
              ...
              {empleados.map((e) => (
                <tr key={e.id}>
                  ...
                  <td className="px-4 py-3 text-right text-primary font-black">
                    ${Number(e.salario_base).toLocaleString("es-CO")}
                  </td>
                  <td className="px-4 py-3 text-center">
                    <div className="flex justify-center gap-2">
                      <button
                        onClick={() =>
                          setPagoModal({
                            id: e.id,
                            nombre: e.nombre,
                            monto: Number(e.salario_base),
                            concepto: `Pago Nómina ${e.nombre}`,
                          })
                        }
                      ...
                      <button
                        onClick={() => setModal(e)}
                        className="p-1.5 bg-gray-50 text-gray-400 rounded-lg hover:text-secondary"
                      >
                        <Pencil size={16} />
                      </button>
                      <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {empleados.map((e) => (
            <div
              key={e.id}
              className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm flex items-center justify-between hover:border-primary transition-colors"
            >
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[10px] font-black uppercase text-secondary bg-secondary/10 px-2 py-0.5 rounded">
                    {e.cargo}
                  </span>
                ...
                <p className="text-primary font-black text-xl mt-3">
                  ${Number(e.salario_base).toLocaleString("es-CO")}
                </p>
              </div>
      ...
      {modal && (
        <ModalOverlay onClose={() => setModal(null)}>
          <h3 className="text-xl font-black mb-1">
            {modal.id ? "Editar" : "Nuevo"} Empleado
          </h3>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <LabelInput
                ...
                value={modal.nombre || ""}
                onChange={(v) => setModal({ ...modal, nombre: v })}
              />
            </div>
            <LabelInput
              label="Documento (CC)"
              value={modal.documento || ""}
              onChange={(v) => setModal({ ...modal, documento: v })}
            />
            <LabelInput
              label="Cargo"
              value={modal.cargo || ""}
              onChange={(v) => setModal({ ...modal, cargo: v })}
            />
            <LabelInput
              label="Teléfono"
              value={modal.telefono || ""}
              onChange={(v) => setModal({ ...modal, telefono: v })}
            />
            <LabelInput
              label="Dirección"
              value={modal.direccion || ""}
              onChange={(v) => setModal({ ...modal, direccion: v })}
            />
            <LabelInput
              label="Fecha Ingreso *"
              type="date"
              value={
                modal.fecha_ingreso ? modal.fecha_ingreso.split("T")[0] : ""
              }
              onChange={(v) => setModal({ ...modal, fecha_ingreso: v })}
            />
            <LabelInput
              ...
              value={
                modal.fecha_nacimiento
                  ? modal.fecha_nacimiento.split("T")[0]
                  : ""
              }
              onChange={(v) => setModal({ ...modal, fecha_nacimiento: v })}
            />
            <LabelInput
              label="Salario Base"
              type="number"
              value={String(modal.salario_base || 0)}
              onChange={(v) => setModal({ ...modal, salario_base: Number(v) })}
            />
            <div className="col-span-2 pt-2 border-t border-gray-100 mt-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <LabelInput
                  label="Banco"
                  value={modal.banco || ""}
                  onChange={(v) => setModal({ ...modal, banco: v })}
                />
                <LabelInput
                  label="N° de Cuenta"
                  value={modal.cuenta_bancaria || ""}
                  onChange={(v) => setModal({ ...modal, cuenta_bancaria: v })}
                />
              </div>
            </div>
          </div>
          <button
            onClick={guardar}
            className="w-full bg-primary text-darkBg font-bold py-3 rounded-lg mt-6 shadow-lg active:scale-95 transition-transform"
          >
        ...
        <ModalOverlay onClose={() => setPagoModal(null)}>
          ...
          <div className="space-y-4">
            <LabelInput
              ...
              onChange={(v) => setPagoModal({ ...pagoModal, concepto: v })}
            />
            <LabelInput
              label="Monto a Pagar"
              type="number"
              value={String(pagoModal.monto)}
              onChange={(v) => setPagoModal({ ...pagoModal, monto: Number(v) })}
            />
            <button
              onClick={registrarPago}
              className="w-full bg-green-500 text-white font-bold py-3 rounded-xl mt-4 shadow-lg shadow-green-200 active:scale-95 transition-transform"
            >
