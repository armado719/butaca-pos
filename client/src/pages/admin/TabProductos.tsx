...
export const TabProductos = () => {
  ...
  const [busqueda, setBusqueda] = useState(
    sessionStorage.getItem("productos_busqueda") || "",
  );
  const [viewMode, setViewMode] = useState<"grid" | "list">(
    (sessionStorage.getItem("productos_view") as any) || "list",
  );
  const [loading, setLoading] = useState(true);

  const [page, setPage] = useState(
    Number(sessionStorage.getItem("productos_page")) || 1,
  );
  ...
  const confirmarEliminar = async () => {
    ...
    try {
      await api.delete(`/admin/productos/${confirmDelete}`);
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {filtrados.map((p) => (
            <div
              key={p.id}
              className="bg-white border border-gray-200 rounded-2xl p-4 flex flex-col shadow-sm hover:shadow-md transition-shadow"
            >
              <div className="flex justify-between items-start mb-2">
                <span className="text-[10px] uppercase font-black px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full">
                  {p.categoria_nombre}
                </span>
                <button onClick={() => toggleDisponible(p.id)}>
                  {p.disponible ? (
              ...
              <p className="font-bold text-gray-800 text-sm mb-1 leading-tight">
                {p.nombre}
              </p>
              ...
              <div className="flex gap-2 mt-3">
                <button
                  onClick={() => setModal(p)}
                  className="flex-1 py-1.5 bg-gray-50 text-gray-400 hover:text-secondary border border-gray-100 rounded-lg flex items-center justify-center transition"
                >
                  <Pencil size={14} />
                </button>
                <button
                  onClick={() => eliminar(p.id)}
                  className="py-1.5 px-2 bg-red-50 text-red-400 hover:bg-red-100 border border-red-100 rounded-lg flex items-center justify-center transition"
                >
                  <Trash2 size={14} />
                </button>
              </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setModal({ ...modal, categoria_id: Number(e.target.value) })
                }
            ...
            <LabelInput
              ...
              onChange={(v) => setModal({ ...modal, descripcion: v })}
            />
            <LabelInput
              label="Precio (COP)"
              type="number"
              value={String(modal.precio || "")}
              onChange={(v) => setModal({ ...modal, precio: Number(v) })}
            />
          </div>
      ...
      <ConfirmDialog
        ...
        message="Esta acción no se puede deshacer. El producto se eliminará permanentemente."
