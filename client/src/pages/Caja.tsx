...
export const CajaUI = () => {
  ...
  const enviarDomicilio = async () => {
    ...
    if (carrito.length === 0)
      return toast("Agrega al menos un producto", "warning");
    setEnviando(true);
    try {
  ...
  const actualizarEstadoDomicilio = async (
    pedidoId: number,
    estadoDomicilio: string,
  ) => {
    try {
      await api.put(`/pedidos/${pedidoId}/estado-domicilio`, {
        estado_domicilio: estadoDomicilio,
      });
      setPedidoActivo((prev) =>
        prev ? { ...prev, estado_domicilio: estadoDomicilio } : null,
      );
      cargarPedidos();
    } catch {
                ...
                {menu.map((cat) => (
                  <div key={cat.id} className="mb-5">
                    <p className="text-xs font-black text-gray-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                      <span className="h-px flex-1 bg-gray-100" /> {cat.nombre}{" "}
              ...
              <div className="w-full md:w-80 flex flex-col overflow-hidden">
                ...
                <div className="p-4 border-t border-gray-100 bg-gray-50">
                  ...
                  <button
                    ...
                    disabled={
                      enviando || !domicilioData || carrito.length === 0
                    }
                    ...
                    style={{
                      background:
                        !domicilioData || carrito.length === 0
                          ? "#e5e7eb"
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-gray-400 text-xs">
                        {p.tipo === "domicilio" ? p.cliente_nombre : p.mesero}
                      </span>
                      <span className="font-black text-gray-800">
                  ...
                  <h2 className="text-lg md:text-xl font-black text-gray-800 flex items-center gap-2 flex-wrap">
                    {pedidoActivo.tipo === "domicilio" ? (
                      <>
                        ...
                        {pedidoActivo.cliente_nombre ?? "Domicilio"}
                      </>
                    ) : (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {pedidoActivo.productos.map((p, i) => (
                      <tr key={i}>
                        <td className="py-3 font-bold text-gray-600 w-12">
                          {p.cantidad}
                        </td>
              ...
              {pedidoActivo.tipo === "domicilio" && (
                <div className="px-5 md:px-8 py-3 border-t border-amber-100 bg-amber-50 flex items-center gap-3 flex-wrap">
                  ...
                  {pedidoActivo.estado_domicilio === "pendiente" && (
                    <button
                      onClick={() =>
                        actualizarEstadoDomicilio(pedidoActivo.id, "en_camino")
                      }
